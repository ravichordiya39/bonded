  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addEditModalLabel">Add Fabric</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
    	<form  method="post" action="{{url('admin/product/config/fabric/save')}}" enctype="multipart/form-data">
		@csrf
		<input type="hidden" name="id" id="id" value="0">
      <div class="modal-body">
       	<div class="form-row">
             <div class="form-group col-12">
                <label>Name of Fabric</label>
...
$(document).ready(function () {
    ...
    var siteurl="{{url('admin/product/config/fabric')}}"
	$(document).on("click", ".edit-data-btn", function () {
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
				$('#addEditModalLabel').text('Edit Fabric');
				console.log(data);
                if (data.status == 1) {
