  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addEditModalLabel">Add Pattern</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
    	<form  method="post" action="{{url('admin/product/config/pattern/save')}}" enctype="multipart/form-data">
		@csrf
...
$(document).ready(function () {
    ...
    var siteurl="{{url('admin/product/config/pattern')}}"
	$(document).on("click", ".edit-data-btn", function () {
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
				$('#addEditModalLabel').text('Edit Pattern');
				console.log(data);
                if (data.status == 1) {
