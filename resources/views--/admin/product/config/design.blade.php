  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addEditModalLabel">Add Design</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
    	<form  method="post" action="{{url('admin/product/config/design/save')}}" enctype="multipart/form-data">
		@csrf
...
$(document).ready(function () {
    ...
    var siteurl="{{url('admin/product/config/design')}}"
	$(document).on("click", ".edit-data-btn", function () {
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
				$('#addEditModalLabel').text('Edit Design');
				console.log(data);
                if (data.status == 1) {
