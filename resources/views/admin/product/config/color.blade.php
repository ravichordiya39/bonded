                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-hover ">
                                ...
                                <tbody>
									...
									<tr>
									<td>{{ $list->name}}</td>
									<td><input type="color" name="color_code" value="{{ $list->color_code}}" readonly disabled> &nbsp;({{ $list->color_code}})</td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addEditModalLabel">Add Color</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
    	<form  method="post" action="{{url('admin/product/config/color/save')}}" enctype="multipart/form-data">
		@csrf
...
$(document).ready(function () {
    ...
    var siteurl="{{url('admin/product/config/color')}}"
	$(document).on("click", ".edit-data-btn", function () {
        ...
        $.ajax({
            ...
            success: function (data, status, xhr) {
				$('#addEditModalLabel').text('Edit Color');
				console.log(data);
                if (data.status == 1) {
                    ...
                    $("#color_code").val(result.color_code);
                    $("#addEditModal").modal({
						 backdrop: 'static',
						 keyboard: false
					});
